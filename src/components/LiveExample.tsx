...
const COMMAND_DEMOS: Record<string, { cmd: string; out: string[] }> = {
  ...
  // バッチ4
  sed: { cmd: "sed 's/http:/https:/g' index.html", out: ["- <a href=\"http://...\">", "+ <a href=\"https://...\">"] },
  awk: { cmd: "awk '{print $1}' access.log", out: ["192.168.1.8", "10.0.0.4"] },
  cut: { cmd: "cut -d, -f1 users.csv", out: ["name", "Alice", "Bob"] },
  tee: { cmd: "npm run build | tee build.log", out: ["Compiled successfully", "（画面表示＋build.logに保存）"] },
  xargs: { cmd: 'find . -name "*.tmp" | xargs rm', out: [] },
  watch: { cmd: "watch -n 2 docker ps", out: ["2秒ごとに docker ps を再実行", "（Ctrl+C で終了）"] },
  date: { cmd: "date +%Y-%m-%d", out: ["2026-07-22"] },
  stat: { cmd: "stat index.html", out: ["Size: 1024   Blocks: 8", "Modify: 2026-07-22 21:00"] },
  file: { cmd: "file logo.png", out: ["logo.png: PNG image data, 512 x 512"] },
  chown: { cmd: "sudo chown you:staff file.txt", out: [] },
  nano: { cmd: "nano .env", out: ["（矢印で移動 / ^O 保存 / ^X 終了）"] },
  vim: { cmd: "vim app.js", out: ["（i 入力 / Esc / :wq 保存終了）"] },
  "npm-ci": { cmd: "npm ci", out: ["added 312 packages in 5s", "(lockfile通りにクリーン導入)"] },
  "npm-publish": { cmd: "npm publish", out: ["+ my-lib@1.0.0"] },
  "npm-version": { cmd: "node -v && npm -v", out: ["v20.11.0", "10.2.4"] },
  nodemon: { cmd: "nodemon server.js", out: ["[nodemon] watching path(s): *.*", "[nodemon] restarting due to changes..."] },
  pm2: { cmd: "pm2 start app.js", out: ["[PM2] App [app] launched (1 instance)", "status: online"] },
  ngrok: { cmd: "ngrok http 3000", out: ["Forwarding  https://abc123.ngrok.io -> http://localhost:3000"] },
  firebase: { cmd: "firebase deploy", out: ["Deploy complete!", "Hosting URL: https://my-app.web.app"] },
  netlify: { cmd: "netlify deploy --prod", out: ["Deploy is live!", "https://my-app.netlify.app"] },
  aws: { cmd: "aws s3 ls", out: ["2026-07-01 my-bucket", "2026-07-15 assets-bucket"] },
  gcloud: { cmd: "gcloud run deploy my-app --source .", out: ["Building and deploying...", "Service URL: https://my-app-xyz.run.app"] },
  heroku: { cmd: "git push heroku main", out: ["-----> Build succeeded", "https://my-app.herokuapp.com deployed"] },
  "git-reflog": { cmd: "git reflog", out: ["e4f5g6h HEAD@{0}: commit: add login", "a1b2c3d HEAD@{1}: checkout: main"] },
  "git-amend": { cmd: 'git commit --amend --no-edit', out: ["[main e4f5g6h] add login", " 2 files changed"] },
  "git-worktree": { cmd: "git worktree add ../hotfix main", out: ["Preparing worktree (checkout 'main')", "HEAD is now at a1b2c3d"] },
  nslookup: { cmd: "nslookup example.com", out: ["Name:    example.com", "Address: 93.184.216.34"] },
  dig: { cmd: "dig example.com +short", out: ["93.184.216.34"] },
  hostname: { cmd: "hostname", out: ["my-macbook.local"] },
  ipconfig: { cmd: "ipconfig", out: ["IPv4 Address . . . : 192.168.1.8", "Subnet Mask  . . . : 255.255.255.0"] },
};
